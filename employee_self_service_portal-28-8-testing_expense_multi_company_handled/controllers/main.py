...
class PortalEmployee(http.Controller):
    ...
    def check_in(self, **post):
        ...
        try:
            ...
            _logger.info(f"Check-in successful for employee {employee.name} at {now}")
    ...
    def check_out(self, **post):
        ...
        try:
            ...
            # TEMPORARILY DISABLED: Validate minimum work duration (at least 30 minutes)
            from datetime import datetime, timedelta
            now = datetime.now()
            check_in_time = fields.Datetime.from_string(last_attendance.check_in)
            
            # TODO: Re-enable 30-minute validation after testing is complete
            # min_duration = timedelta(minutes=30)
            # if (now - check_in_time) < min_duration:
    ...
    def portal_attendance_history(self, **kwargs):
        ...
        if kwargs.get('success') == 'checked_in':
            ...
            error_message = "Check-in not allowed at this time (6 AM - 11 PM only)."
    ...
    def _get_attendance_analytics(self, employee, month, year):
        ...
        # Group attendances by day for accurate day counting
        ...
        for day, day_attendances in attendance_by_day.items():
            ...
            first_check_in = day_attendances[0].check_in
        ...
        for day, day_attendances in attendance_by_day.items():
            ...
            if any(att.check_out and att.check_out.time() < early_threshold for att in day_attendances):
                early_departures += 1
        ...
        week_attendances = request.env[HR_ATTENDANCE_MODEL].sudo().search([
            ('employee_id', '=', employee.id),
            ('check_in', '>=', week_start.strftime('%Y-%m-%d 00:00:00')),
    ...
    def _get_enhanced_dashboard_data(self, employee):
        ...
        # Enhanced attendance data - get ALL attendance records for today
        today = date.today()
        ...
        week_attendance = request.env[HR_ATTENDANCE_MODEL].sudo().search([
            ('employee_id', '=', employee.id),
            ('check_in', '>=', week_start.strftime('%Y-%m-%d 00:00:00')),
            ('check_in', '<=', week_end.strftime('%Y-%m-%d 23:59:59'))
        ])
        ...
        performance_metrics = {
            'attendance_rate': self._calculate_attendance_rate(employee, today),
    ...
    def _calculate_attendance_rate(self, employee, today):
        ...
        # Get unique days
        attended_days = len(set(att.check_in.date() for att in attendance_records))
        
        return (attended_days / working_days * 100) if working_days > 0 else 0
    ...
    def _validate_expense_data(self, post):
        ...
        try:
            ...
            if amount <= 0:
                ...
                errors.append('Amount cannot exceed 50,000.')
        except (ValueError, TypeError):
